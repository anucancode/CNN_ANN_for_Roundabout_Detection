...
trainY = labels(idx(1:round(P*s)),1:2);
testY = labels(idx(round(P*s)+1:end),1:2);
...
for i = 1 : length(testY)
    minDiff = 10;
    diff = ((testY(i,1)-prediction(i,1))^2+(testY(i,2)-prediction(i,2))^2)^0.5;
    if(diff<minDiff)
        count = count+1;
    end
end
...
for i = 1 : length(imdsTest.Files)
    ...
    img = insertShape(img,"circle",[prediction(i,1), prediction(i,2), testYfull(i,3)],LineWidth=2);
